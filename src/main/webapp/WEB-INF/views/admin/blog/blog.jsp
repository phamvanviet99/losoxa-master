                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <!-- DATA TABLE -->
                            <h3 class="title-5 m-b-35">Quản lý blog</h3>
                            <div class="table-data__tool">
                                <a href="<c:url value="/admin/blog/create"/>">
                                    <div class="table-data__tool-right">
                                        <c:forEach items="${currentRole}" var="currentRole">
                                            <c:if test="${currentRole == 'CREATE_BLOG'}">
                            ...
                            <form action="<c:url value="/admin/blog"/> " id="formSubmit" method="get">
                                <div class="table-responsive table-responsive-data2">
                                    <table class="table table-data2">
                                        ...
                                        <tbody>
                                        <c:forEach items="${blog}" var="blog" varStatus="itr">
                                            <tr class="tr-shadow">
                                                <td>
                                                    <label class="au-checkbox">
                                                        <input type="checkbox">
                                                ...
                                                <td>${itr.index +1}</td>
                                                <td><img src="<c:url value="${blog.thumbnail}" />"
                                                         style="height: 90px; width: 90px;" alt=""></td>
                                                <td>${blog.title}</td>
                                                <td>${blog.createdAt}</td>
                                                <td>${blog.createdBy}</td>
                                                <td>${blog.modifiedAt}</td>
                                                <td>${blog.modifiedBy}</td>
                                                <td>
                                                    <div class="table-data-feature">
                                                        <c:forEach items="${currentRole}" var="role">
                                                            <c:if test="${role == 'UPDATE_BLOG'}">
                                                                <a href="<c:url value="/admin/blog/update/${blog.id}" />">
                                                            ...
                                                            <c:if test="${role == 'DELETE_BLOG'}">
                                                                <a onclick="return confirm('Bạn có muốn xóa không?');"
                                                                   href="<c:url value="/admin/blog/delete/${blog.id}" />">
